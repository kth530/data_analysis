...
- **SQL** — 추출·집계·분류·조인·윈도우. 노트북 03~07은 짝이 되는 [`sql/`](sql/) 파일(`-- name:` 마커)에서 쿼리를 호출한다. RFM 등급·세그먼트 분류는 `rfm_scored` VIEW(CASE WHEN)로 처리.
...
- DB: MySQL — 노트북 03~07은 [`sql/`](sql/)의 쿼리를 `load_queries()`로 불러 실행
